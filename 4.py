...
def download_unsplash_images(keyword: str, total: int = 100):
    folder = f"{keyword}_images"
    os.makedirs(folder, exist_ok=True)
    driver = webdriver.Chrome()
    driver.get(f"https://unsplash.com/s/photos/{keyword.lower()}")
    time.sleep(3)
    ...
    for i, url in enumerate(image_urls, start=1):
        try:
            ...
            img.save(os.path.join(folder, f"{keyword.lower()}_{i}.jpg"))
            print(f"[{keyword}] 📥 Saved {keyword.lower()}_{i}.jpg")
        except Exception as e:
            print(f"[{keyword}] ❌ Error saving {keyword.lower()}_{i}.jpg — {e}")

    driver.quit()

# Example usage:
download_unsplash_images("Macaw", total=100)
